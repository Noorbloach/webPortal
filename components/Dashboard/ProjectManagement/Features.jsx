    ...
    <Grid container justifyContent="center" rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 2 }}>
      {FeaturesData.map((feature) => (
        <Grid item xs={12} sm={6} md={6} lg={6} xl={3} key={feature.id}>
          <Card sx={{ boxShadow: "none", borderRadius: "10px", p: "20px 15px", mb: "15px" }}>
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Box>
                  <Typography sx={{ fontSize: "13px" }}>{feature.subTitle}</Typography>
                  <Typography variant="h1" sx={{ fontSize: 25, fontWeight: 700, mt: '4px' }}>
                    {feature.title}
                  </Typography>
                </Box>
              </Box>
